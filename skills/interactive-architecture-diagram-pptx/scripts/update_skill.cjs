...
const PACKAGE_ID = "@user_bddf3fe6/contextweave-interactive-architecture-pptx";
const INSTALLER_URL =
...
function printHelp() {
  process.stdout.write(
    [
      "Update ContextWeave Interactive Architecture PPTX Skill.",
      "",
      "Usage:",
...
function download(url, destination, redirects = 0) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = transport.get(parsed, { headers: { "User-Agent": "contextweave-pptx-skill-updater/1" } }, (response) => {
...
async function installWithBundledCli(skillsDir, requiredVersion) {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "contextweave-pptx-skill-update-"));
  ...
  try {
    ...
    if (!runInstall(bundledCli, skillsDir, requiredVersion)) {
      throw new Error("SkillHub installer failed to update the ContextWeave PPTX Skill");
    }
  } finally {
...
async function main() {
  ...
  process.stdout.write("==========================================\n");
  process.stdout.write("Updating ContextWeave Interactive Architecture PPTX Skill\n");
  process.stdout.write("==========================================\n");
...
main().catch((error) => {
  process.stderr.write(`ContextWeave PPTX Skill update failed: ${error.message || error}\n`);
