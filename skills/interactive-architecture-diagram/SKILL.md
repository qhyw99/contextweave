...
2. **意图挖掘**：从用户自然语言中提取展示意图（见 §三）。
3. **层级规划**：判断是否过于复杂，决定单图 / scenarios / layers（见 §5.1）。
...
| 分层结构（"按接入层/应用层/数据层上下排"） | 指定复杂的自定义布局算法 |
...
- `generate_contextweave.cjs`：基于 `input_file` 生成；`--enable_plan true` 启用大纲规划模式（适合特别复杂的逻辑结构）
