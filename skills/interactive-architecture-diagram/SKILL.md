...
当用户要求在图中的对象（无论是模块节点还是连线）上关联本地文件路径时，执行**两步法**：
1. **结构生成**：仅根据语义生成图结构，忽略链接要求。
2. **批量注入**：调用 `edit_contextweave.cjs` 时，通过 `--base_path <当前工作区路径>` 传入绝对前缀。在 `# Request` 中输出如下的纯语义 JSON 格式指令。请直接使用自然语言描述目标对象（如节点名称或连线描述），后端大模型会自动将其翻译为底层的精确路径并完成注入：
   ```json
   {
     "links": [
       { "targets": ["模块A", "模块B"], "link": "./src/module.py" },
