...
普通单图只需先理解三件事，后面的规则都是它们的延伸。

### 1. 把背景说完整

云端看不到本地文件、会话历史或未解释的缩写。调用脚本前，必须把绘图所需信息整理成可独立理解的纯文本。

| 用户输入 | 必须执行的处理 |
|---|---|
| “参考 `/path/to/x`” | 先读取文件，把核心逻辑、职责和关系写入 `# Request` |
| 未解释的专有名词或缩写 | 补充角色、所属层级、关键动作和上下游 |
| “基于上一张图修改” | 携带现有 CW 与上一轮 `session_id`；操作方式见 [高级操作](references/advanced-operations.md) |

禁止把未释义的术语直接堆成节点，也禁止把本地路径原样转发给后端。

### 2. 把关系说清楚

- 图必须表达层级、依赖、因果或流转关系，而不是只展示一组名词。
- 每条关键关系都应能复述成明确语句，例如“A 调用 B”“C 触发 D”。
- 不能用“两个元素靠得近”代替关系定义。
- 即使暂时隐藏文字标签，分组、方向和连线也应保留核心逻辑。

### 3. 一张图只回答一个核心问题

宏观图讲全局骨架，中观图讲模块协作，微观图讲执行细节。先确定焦点和抽象层级，再决定画多细；不要把所有内容塞进一张图。

如果一句话无法说清这张图要回答什么，或用户要求按模块、层级、链路分别查看，先进入“多视图触发与确认门”，不要直接生成。

## 二、普通单图：六步完成

### 1. 解析材料

识别核心问题、信息焦点与需要读取的文件。只读取用户明确指定且与绘图有关的内容，并按第一条原则补全上下文。
...
| # | 反模式 | 违反 | 正确做法 |
|---|---|---|---|
| 1 | `# Request` 中出现“请参考文件 /path/to/x” | 核心原则 1 | 自行读取文件，拍平为纯文本写入 `# Request` |
| 2 | 术语未释义直接作为节点或分组标签 | 核心原则 1 | 补全角色、层级、动作、上下游后再出图 |
| 3 | 修改已有图时不带 `# CW` | 核心原则 1 | 将现有 CW 放入 `# CW` 并复用 `session_id` |
| 4 | 用“元素靠得近”表达关系 | 核心原则 2 | 使用明确、带方向且可复述的关系 |
| 5 | 一张图塞入所有细节 | 核心原则 3 | 按受众确定层级，复杂时进入多视图判断 |
