...
**适用场景：**
- 需要将复杂的业务逻辑、代码结构、或者长文本转换为直观的架构图、流程图或思维导图。
- 用户要求对代码逻辑进行图解。
...
当遇到复杂架构、多业务链路或庞大系统时，**禁止**试图在一张无层次的图中塞入所有细节（面面俱到），必须通过以下机制进行拆分，并遵循对应的决策表：

| 用户意图 | 应选机制 |
|---|---|
| "同一架构上高亮/区分不同链路"（如区分 Query 链路和 Callback 链路） | `scenarios`（场景/链路拆分） |

### Scenarios (场景/链路拆分)
- **场景抽象与单一数据源**：**必须**使用 ContextWeave/D2 的 `scenarios` 原生语法。**禁止**复制或拼接多个完整的图形代码。应首先定义一个完整的“基础架构图（Base Layer）”，然后在同一个代码块底部使用 `scenarios` 定义各个视图的增量覆盖。
- **场景构建规范**：
  - 基础层必须包含所有节点和连线。
...
1. **解析需求**：识别用户想表达的核心问题、受众和信息密度。必须先自行读取本地文件并拍平（Flatten）为纯文本。
2. **层级与视图规划**：判定图表是否过于复杂（连线过多）。如果过于复杂，必须使用 `scenarios` 规划不同链路进行拆分。
3. **结构决策**：基于哲学层原则选择图组织方式并显式定义关系。
4. **请求编排落盘**：将结构化意图组织为文件并写入 `input_file`（必须为绝对路径）。若为多场景任务，需在 `# Request` 中明确指出场景关系。
5. **基于文件生成**：执行 `node scripts/generate_contextweave.cjs --input_file "<绝对文件路径>"`。
6. **结果校验与回填**：获取 `session_id`，检查结构是否满足“论证性、关系显式、同构可读”，并将结果交付用户。
...
- **零配置开箱即用**：本 Skill 内置了默认的匿名调用凭据。**严禁**在交互过程中向用户索要 API Key、**严禁**要求用户配置环境变量、**严禁**提示需要鉴权。
- **数据路由**：请求将默认发送至官方服务器（`https://pptx.chenxitech.site`），只发送与绘图相关的数据。
- **文件访问**：只读取明确指定的输入文件；禁止遍历用户目录、工作区或无关配置文件；所有文件路径必须是绝对路径且被严格限制在当前执行工作区目录范围内。
