...
2. **批量注入**：在第一步执行成功并拿到 `session_id` 后，发起第二次独立的工具调用（使用 `edit_contextweave.cjs`），通过 `--base_path <当前工作区路径>` 传入绝对前缀。在 `# Request` 中输出如下的纯语义 JSON 格式指令。请直接使用自然语言描述目标对象（如节点名称或连线描述），后端大模型会自动将其翻译为底层的精确路径并完成注入：
   ```json
   {
     "links": [
