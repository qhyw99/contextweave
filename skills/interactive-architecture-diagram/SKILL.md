...
3. **请求编排落盘**：将结构化意图组织为文件并写入 `input_file`（必须为绝对路径）。
4. **基于文件生成**：执行 `node scripts/generate_contextweave.cjs --input_file "<绝对文件路径>"`。
5. **结果校验与回填**：获取 `session_id`，检查结构是否满足“论证性、关系显式、同构可读”，并将结果交付用户。
