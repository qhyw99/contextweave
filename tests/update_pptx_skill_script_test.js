...
const skillRoot = path.join(repoRoot, "skills", "interactive-architecture-diagram-pptx");
const updater = path.join(skillRoot, "scripts", "update_skill.cjs");
const packageId = "@user_bddf3fe6/contextweave-interactive-architecture-pptx";

function runUpdater(script, extraArgs = [], env = process.env) {
...
const expectedTarget = path.join(
  ...
  "skills",
  "@user_bddf3fe6",
  "contextweave-interactive-architecture-pptx"
);
const targetExistedBefore = fs.existsSync(expectedTarget);
const dryRun = runUpdater(updater, ["--dry-run"]);
...
const versionedDryRun = runUpdater(updater, ["--dry-run", "--required-version", "1.2.8"]);
...
const customRoot = path.join(os.tmpdir(), "contextweave-pptx-custom-agent-skills");
const customDryRun = runUpdater(updater, ["--dry-run", "--skills-dir", customRoot]);
...
assert.strictEqual(
  ...
  path.join(path.resolve(customRoot), "@user_bddf3fe6", "contextweave-interactive-architecture-pptx")
);

const unsafeRoot = path.parse(repoRoot).root;
const unsafeDryRun = runUpdater(updater, ["--dry-run", "--skills-dir", unsafeRoot]);
assert.notStrictEqual(unsafeDryRun.status, 0);
assert.match(unsafeDryRun.stderr, /Refusing to use a filesystem root/);

const layoutSandbox = fs.mkdtempSync(path.join(os.tmpdir(), "contextweave-pptx-layout-"));
try {
  ...
  const namespacedUpdater = path.join(namespacedScriptsDir, "update_skill.cjs");
  fs.mkdirSync(namespacedScriptsDir, { recursive: true });
  fs.copyFileSync(updater, namespacedUpdater);
  fs.writeFileSync(
  ...
  const namespacedDryRun = runUpdater(namespacedUpdater, ["--dry-run"]);
  ...
  assert.strictEqual(path.resolve(namespacedPayload.skills_dir), path.join(layoutSandbox, "skills"));
...
const updateSandbox = fs.mkdtempSync(path.join(os.tmpdir(), "contextweave-pptx-update-"));
try {
  ...
  fs.copyFileSync(updater, sandboxUpdater);
  fs.writeFileSync(
  ...
  fs.writeFileSync(
    ...
    [
      ...
      'const target = path.join(args[dirIndex + 1], "@user_bddf3fe6", "contextweave-interactive-architecture-pptx");',
  ...
  const updateEnvironment = { ...process.env, FAKE_SKILLHUB_LOG: fakeLog };
  const automaticUpdate = runUpdater(
    sandboxUpdater,
    ["--required-version", "1.1.0", "--skillhub-cli", fakeCli],
    updateEnvironment
  );
  assert.strictEqual(automaticUpdate.status, 0, automaticUpdate.stderr);
  assert.match(automaticUpdate.stdout, /Installed version: 1\.1\.0/);
  const installArgs = JSON.parse(fs.readFileSync(fakeLog, "utf8"));
  assert.deepStrictEqual(installArgs.slice(0, 2), ["install", packageId]);
  ...
  const wrongVersionUpdate = runUpdater(
    sandboxUpdater,
    ["--required-version", "1.1.0", "--skillhub-cli", fakeCli],
    { ...updateEnvironment, FAKE_SKILLHUB_VERSION: "1.0.0" }
  );
  assert.notStrictEqual(wrongVersionUpdate.status, 0);
  assert.match(
    wrongVersionUpdate.stderr,
    /Installed version 1\.0\.0 does not match required version 1\.1\.0/
  );
} finally {
  fs.rmSync(updateSandbox, { recursive: true, force: true });
}

console.log("update_pptx_skill_script_test: ok");
